...
export const joinCell = async (cellId: number): Promise<void> => {
	const response = await secureFetch(
		`${apiEndpoints.cells.joinCell(cellId)}`,
		{
			method: "POST",
		},
	);
...
export const updateMemberStatus = async (
	...
	const response = await secureFetch(
		`${apiEndpoints.cells.updateMemberStatus(cellId)}`,
		{
			method: "PUT",
...
export const removeCellMembers = async (
	...
	const response = await secureFetch(
		`${apiEndpoints.cells.removeCellMembers(cellId)}`,
		{
			method: "POST",
			headers: { "Content-Type": "application/json" },
			body: JSON.stringify({
				cell_id: cellId,
				people: memberIds.map((id) => ({ id })),
				user: personId,
			}),
		},
	);
...
export const addCellMembers = async (cellId: number, people: Person[]) => {
	const response = await secureFetch(
		`${apiEndpoints.cells.addMembers(cellId)}`,
		{
			method: "POST",
			headers: { "Content-Type": "application/json" },
			body: JSON.stringify({
