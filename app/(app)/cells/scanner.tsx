...
const ScannerPage = () => {
	...
	// Auto-navigate if already granted (add delay to ensure native permission is applied)
	useEffect(() => {
		if (!isPermissionGranted) return;
		console.log("isPermissionGranted", isPermissionGranted);
		const cellIdParam = Array.isArray(cell_id) ? cell_id[0] : cell_id;
		
		// Small delay to ensure native camera permission is fully processed
		const timer = setTimeout(() => {
			router.replace({
				pathname: "/(app)/cells/qrScan",
				params: cellIdParam ? { cell_id: cellIdParam } : undefined,
			});
		}, 300);
