import { ChevronLeft, CircleQuestionMark, FileText, LockKeyhole, Mail, Moon, SquareArrowRight } from "lucide-react-native"
    ...
    <View className="flex-1 bg-gray-900">
      ...
      <View className="flex-1 px-4 pt-6">
        {settingsOptions.map((option, index) => (
          ...
          >
            {/* Icon */}
            <View className="w-10 h-10 items-center justify-center mr-4">{option.icon}</View>

            {/* Content */}
            <View className="flex-1">
              <Text className={`text-lg font-medium ${option.isDestructive ? "text-red-500" : "text-white"}`}>
                {option.title}
              </Text>
