...
export const useCellAttendanceStatsQuery = (cellId: number, enabled = true) => {
	return useQuery({
...
export const useAddCellMembersMutation = (cellId: number) => {
	const qc = useQueryClient();

	return useMutation({
		mutationFn: (payload: { cellId: number; people: Person[] }) =>
			addCellMembers(payload.cellId, payload.people),
		onSuccess: (res, variables) => {
			qc.setQueryData(["cells", variables.cellId], (oldData: Cell) => {
