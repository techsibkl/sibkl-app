...
import { AbilityBuilder, type AnyAbility, createMongoAbility } from "@casl/ability";
...
export function defineAbilityFor(person: Person): AnyAbility {
  ...
  if (
    ...
    !person?.roles?.every(role => Object.values<string>(Role).includes(role))
  ) {
    // No person, no roles, or invalid roles
    return build();
  }

  if (person?.roles?.includes(Role.SENIOR_PASTOR) || person?.roles?.includes(Role.SUPER_ADMIN)) {
    can("manage", "all");
  }

  if (person?.roles?.includes(Role.CELL_ADMIN) || person?.roles?.includes(Role.CELL_COORDINATOR)) {
    can("manage", "all");
  ...
  if (person?.roles?.includes(Role.DISTRICT_ADMIN)) {
    ...
    // Duplicates
    can(["read", "merge", "archive"], "Duplicates", { district_id: { $in: person.admin_district_ids } });
    ...
    can(["read", "update", "delete", "add"], "Flow", { district_id: { $in: person.admin_district_ids } });
  ...
  if (person?.roles?.includes(Role.DISTRICT_PASTOR)) {
    ...
    can("create", "Flow");
    can("read", "Flow", { district_id: null, get_public: true }); // separate read permissions (some flows allow public to read)
    can(["read", "update", "delete", "add"], "Flow", { district_id: { $in: person.pastor_district_ids } });
    can(["read", "update", "assign"], "PeopleFlow", {
      district_id: { $in: person.pastor_district_ids }
    });
